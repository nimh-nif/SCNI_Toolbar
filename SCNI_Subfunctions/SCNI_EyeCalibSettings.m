...
if Success < 1 || ~isfield(Params, 'Eye')                                         	% If the parameters could not be loaded...
    ...
    Params.Eye.Cal.Labels       = {'Left','Right','Version'};
...
for p = 1:numel(Fig.PanelNames)
    ...
    for n = 1:numel(PanelStructs{p}.Labels)
        ...
        if p == 1 && n >= 4
            for xy = 1:2
            	Fig.UIhandle(p,n,xy) = uicontrol(  'Style', PanelStructs{p}.Style{n},...
            ...
            Fig.UIhandle(p,n,1) = uicontrol(  'Style', PanelStructs{p}.Style{n},...
...
set(Fig.UIhandle(2,9,1), 'backgroundcolor', Params.Eye.MarkerColor);
...
%% =========================== SUBFUNCTIONS ===============================
    ...
    function OptionSelect(hObj, Event, Indx)
        switch Indx
            ...
            case 2  %============ Save parameters to file
                ...
                fprintf('Eye calibration parameters saved to %s!', Params.File);
    ...
    function UpdateParams(hObj, Evnt, Indx1, Indx2)
        ...
        %============= Panel 2 controls for directory selection
        if Indx1 == 2 && Indx2 == 7                         %============= Change stimulus directory
            ...
            set(Fig.UIhandle(2,7,1),'string',Params.Eye.MarkerDir);
     
        elseif Indx1 == 1 && Indx2 == 3                     %============= Different eye selected
            Params.Eye.EyeToUse = get(hObj,'value');
            set(Fig.UIhandle(1,1,1), 'string', num2str(Params.Eye.XYchannels{Params.Eye.EyeToUse})); 
            set(Fig.UIhandle(1,2,1), 'string', num2str(Params.Eye.Pupilchannels{Params.Eye.EyeToUse}));
            for xy = 1:2
                set(Fig.UIhandle(1,4, xy), 'string', num2str(Params.Eye.Cal.Offset{Params.Eye.EyeToUse}(xy)));
                set(Fig.UIhandle(1,5, xy), 'string', num2str(Params.Eye.Cal.Gain{Params.Eye.EyeToUse}(xy)));
                set(Fig.UIhandle(1,6, xy), 'value', Params.Eye.Cal.Sign{Params.Eye.EyeToUse}(xy));
            end
    ...
    function Params = UpdateEye(Params)
        
        for n = [1,2,4,5,6]
            NewValue = eval(sprintf('Params.Eye.%s{%d};', Fig.PanelVars(1).Fieldnames{n}, Params.Eye.EyeToUse));
            if n < 6
                set(Fig.UIhandle(1,n,xy), 'string', num2str(NewValue)); 
            else
                InvertValue = NewValue==-1;
                set(Fig.UIhandle(1,n,xy), 'value', InvertValue); 
            end
        end
