function [FrameTextures] = GenerateDynamicNoise(Dot, Display)

%======================== GenerateDynamicNoise.m ==========================
%
% INPUTS:
...
% 05/03/2014 - Adapted from GenerateRDK.m
%     ___  ______  __   __
%    /   ||  __  \|  \ |  \    APM SUBFUNCTIONS
...
%================= 
Dot.Type = 2;
DotSizeMin = min(Dot.DotSize);
DotSizeRange = abs(diff(Dot.DotSize));
xpos = randi(Dot.Window(1), [Dot.NoFrames, Dot.NoDots]);
ypos = randi(Dot.Window(2), [Dot.NoFrames, Dot.NoDots]);
% Dot.Colour = (randi(2, [1, Dot.NoDots, Dot.NoFrames])-1)*255;
% Dot.Colour = repmat(Dot.Colour,[3,1,1]);
Dot.Colour = ones(3,Dot.NoDots,Dot.NoFrames)*255;
Dot.Sizes = (rand([Dot.NoFrames, Dot.NoDots])*DotSizeRange)+DotSizeMin;
...
for Frame = 1:Dot.NoFrames
    FrameTextures(Frame) = Screen('MakeTexture', Display.win, TextureBackground(:,:,1), Dot.DrawAngle);
    Screen('DrawDots', FrameTextures(Frame), [xpos(Frame,:); ypos(Frame,:)], Dot.Sizes(Frame,:), Dot.Colour(:,:,Frame), [0 0], Dot.Type);
    
%   	RDKframes(Frame) = Screen('MakeTexture', Display.win, TextureBackground(:,:,1), Dot.DrawAngle);
%     Screen('DrawDots', RDKframes(Frame), [xposVis(Frame,:); ypos(1,:)], Dot.Sizes, Dot.Colour, [0 0], Dot.Type);
%     
%     %================ Check dot lifetimes
...
%     xpos(Frame+1,:) = xpos(Frame,:)+PixPerFrame;            % Advance dot positions
%     xpos(Frame+1, xpos(Frame+1,:)>Dot.Window(1)) = xpos(Frame+1, xpos(Frame+1,:)>Dot.Window(1))-Dot.Window(1);       % Reset positions of dots moving outside of the window
%     xposVis(Frame+1,:) = xpos(Frame+1,:);                   % 
%     for i = 1:Dot.Num
%         if sqrt((xpos(Frame,i)-radius)^2 + (ypos(1,i)-radius)^2) > radius;
%             xposVis(Frame,i) = nan;
%         end
%     end
end