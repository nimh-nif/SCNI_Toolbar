function [CalWin] = EyelinkCalibration(initials, Trial, SessionDir, ReturnWin, Display)
% [CalWin] = EyelinkCalibration(initials, Trial, SessionDir, ReturnWin)
...
TargetSize = round(0.5*Display.Pixels_per_deg(1));        
...
FixBoxRect = round([(x-FixBox/2) (y-FixBox/2) (x+FixBox/2) (y+FixBox/2)]);         % Fixation window  
...
DisplayDeg = (Display.Rect([3 4])/2)/Display.Pixels_per_deg(1); % Calculate display size in degrees
Offsets = repmat(floor(DisplayDeg)-TargetEcc,[1,2]);          	% Set horizontal and vertical target offset intervals (degrees)   
...
for n = 1:numel(TargetPos(:,1))
    TargetOffset(n, :) = ([1 0 1 0]*TargetPos(n,1) + [0 1 0 1]*TargetPos(n,2))*Display.Pixels_per_deg(1);
...
Text.String = 'Saving eye movement data...';
DisplayText(Text, Display);
