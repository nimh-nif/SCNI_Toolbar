...
Fig.UItransform.Labels      = {'Present fullscreen','Background color','Retinal subtense (deg)','Image rotation (deg)','Image contrast (%)','Audio on?','Audio volume (%)','Playback rate (%)'};
Fig.UItransform.Style       = {'checkbox','PushButton','Edit','Edit','Edit','Checkbox','Edit','Edit'};
...
Fig.UItransform.Enabled     = [1, 1, ~Params.Movie.Fullscreen, 1, 1, 1,1,1,1];
Fig.UItransform.Ypos      	= [(Fig.PannelHeights(2)-50):-20:10]*Fig.DisplayScale;
Fig.UItransform.Xwidth     	= [180, 200]*Fig.DisplayScale;

Fig.UIpresent.Labels        = {'Run duration (s)', 'Duration per movie (s)', 'Inter-stim interval (s)', 'Loop movie?', 'Fixation marker','Fixation size (deg)','Fixation color','Gaze contingent reward','Gaze rect border (deg)','Pre-calibrate','First frame dur (ms)'};
Fig.UIpresent.Style        	= {'Edit','Edit','Edit','Checkbox','Popupmenu','Edit','PushButton','Checkbox','Edit','Checkbox','Edit'};
...
Fig.UIpresent.Values        = {[],[],[],Params.Movie.Loop, Params.Movie.FixType,[],[],Params.Movie.FixRequired,[],Params.Movie.PreCalib,[]};
Fig.UIpresent.Enabled       = [1,1,1,1,1,0,0,1,Params.Movie.FixRequired,1,1];
...
Fig.PanelVars(2).Fieldnames     = {'Fullscreen','Background','SizeDeg','Rotation','Contrast','AudioOn','AudioVol','Rate'};
Fig.PanelVars(3).Fieldnames     = {'RunDuration','Duration','ISI','Loop','FixType','FixSize','','FixRequired','GazeRectBorder','PreCalib','FirstFrameDur'};
...
set(Fig.UIhandle(3,ColIndx), 'BackgroundColor', Params.Movie.FixColor, 'Callback',{@ChangeFixColor, 1});
set(Fig.UIhandle(2,2), 'BackgroundColor', Params.Movie.Background, 'Callback',{@ChangeFixColor, 2});
...
%% ========================== SUBFUNCTIONS ================================
    ...
    function ChangeFixColor(hObj, Evnt, Indx)
        Color = uisetcolor;
        if numel(Color>1)
