function [m, s, c] = SCNIAudio_settings(window, screenRect, refreshrate)   
...
if ExpType == 1 %====================== MonkeyVocalizations
 	c.StimDir{1} = fullfile(Append, '/khandhadiaap/AudioVisual Stim/Romanski/Macaque_video/Audio/');
    c.StimDir{2} = fullfile(Append, '/khandhadiaap/AudioVisual Stim/Romanski/Macaque_video/Audio/Controls');
    c.BckgrndDir{1} = [];
    c.StimPerCond  	= 15; 
    
end
c.FileFormat    = '.wav';                                       % Stimulus file format
...
c.Reward_MeanDur    = 3;                                            % Average duration between reward deliveries (seconds)
...
c.InitialFixTime     =.5;                                           %The time the monkey has to fixate before the stimulus 
...
c.output_prefix         = 'SCNIAudio';                               % Define the prefix for the Output File
c.protocol_title        = 'SCNIAudio';                      % Define Banner text to identify the experimental protocol
