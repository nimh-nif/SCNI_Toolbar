function [m, s, c] = SCNI_fixcal_settings(window, screenRect, refreshrate)   
...
c.FixEccentricity   = 6;                           % Eccentricity of non-central targets (degrees visual angle from center)
...
c.StimDuration      = 0.5;                                        	% Stimulus on duration (seconds)
c.ISI               = 1;                                        	% Inter-stimulus interval duration (seconds)
c.ITI               = 4;                                            % Inter-trial interval duration (seconds)
c.TimeToFix         = 0.3;                                          % Maximum allowed duration between fixation target onset and valid fixation
c.FixDur            = 0.2;                                          % Minimum duration of a valid fix
...
c.Fix_Color          = [255,255,255];                                 	% Color of main fixation marker component (RGB)
c.Fix_WinRadius      = 10;                                         	% Radius of fixation window (degrees visual angle)
c.Fix_MarkerSize     = 0.7;                                           % Diameter of fixation marker (degrees visual angle)
c.Fix_LineWidth      = 3;                                           % Line width (pixels)
c.MaxFixBreakDur     = 100;             
% Maximum duration allowed to break fixation (ms)
...
c.Col_bckgrndRGB    = [128,128,128]*0;        % Stimulus background color = mid-grey
