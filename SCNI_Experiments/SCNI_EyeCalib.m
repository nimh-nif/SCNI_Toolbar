...
Params.Run.MaxTrialDur          = Params.Eye.StimPerTrial*(Params.Eye.Duration+Params.Eye.ISIms)*10^-3;
...
KeyNames                    = {'X','R','S','C'};         
KeyFunctions                = {'Stop','Reward','Sound','Center'};
...
switch Params.Eye.MarkerType
    ...
    case 2   	%============ Image fixation marker
        Params.Eye.ImgTex = SCNI_LoadImages(Params);        
...
while Params.Run.TrialCount < Params.Eye.TrialsPerRun && Params.Run.ExpQuit == 0
    ...
    for StimNo = 1:Params.Eye.StimPerTrial                              % Loop through stimuli for this trial
        ...
        LocIndx     = Params.Eye.Target.LocationOrder(StimNo);
        ...
        FixRectMonk    = FixRectExp + Params.Display.Rect([3,1,3,1]);
        ...
        while (GetSecs-Params.Run.StimOnTime) < Params.Eye.Duration/10^3 && Params.Run.ExpQuit == 0
            ...
            %SCNI_SendEventCode(Stim, LocIndx);                                                                  % Send stimulus position number to neurophys. system 
