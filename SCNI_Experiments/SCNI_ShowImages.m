...
function EndRun = CheckKeys(Params)
    ...
    if keyIsDown && secs > Params.Run.LastPress+0.1                              	% If key is pressed and it's more than 100ms since last key press...
        ...
        if keyCode(Params.ImageExp.Keys.Stop) == 1                                  % Experimenter pressed quit key
            ...
            SCNI_EndRun(Params);
            EndRun = 1;
        elseif keyCode(Params.ImageExp.Keys.Reward) == 1                          	% Experimenter pressed manual reward key
            Params = SCNI_GiveReward(Params);
        elseif keyCode(Params.ImageExp.Keys.Sound) == 1                             % Experimenter pressed play sound key
